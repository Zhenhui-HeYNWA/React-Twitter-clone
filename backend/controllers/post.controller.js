...
export const createPost = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.states(404).json({ message: 'User not found' });
    }
    if (!text && !img) {
      return res.states(400).json({ error: 'Post must have text or image' });
    }
...
export const likeUnlikePost = async (req, res) => {
  try {
    ...
    if (userLikedPost) {
      ...
      await User.updateObe({ _id: userId }, { $pull: { likedPosts: postId } });
